...
### Added
- **Browser-based first-run setup, at `/setup`.** A fresh deploy no
  longer needs shell/`DATABASE_URL` access to create its first admin
  account — the point of a one-click Render deploy was defeated if that
  was still the only way in. `GET /api/auth/setup-status` /
  `POST /api/auth/setup` (`backend/src/routes/auth.js`) are public but
  self-disable permanently the moment any user exists (re-checked inside
  the handler itself, not just trusted from an earlier request), and are
  rate-limited the same as `/login`. Visiting `/login` on an instance
  with no admin yet redirects straight to `/setup` automatically —
  nothing to know or type. `node prisma/createAdmin.js` still works
  exactly as before for anyone who prefers a shell.
- `APP_BASE_URL` (used only to build the SSO callback URL) now falls
  back to Render's own `RENDER_EXTERNAL_URL` when unset
  (`backend/src/utils/urls.js`) — SSO works out of the box on Render's
  default `onrender.com` URL with nothing to configure. Only needs
  setting explicitly for a custom domain. `SESSION_SECRET` and
  `FIELD_ENCRYPTION_KEY` were already auto-generated by `render.yaml`
  (`generateValue: true`, since v2.1.0) — no change needed there, just
  confirming it for anyone checking off the onboarding-friction list.
